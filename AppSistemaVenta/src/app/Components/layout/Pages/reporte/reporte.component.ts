...
export class ReporteComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    // Buscar las ventas de los últimos 30 días al inicializar el componente
  ...
  buscarVentas(fechaInicio?: string, fechaFin?: string): void {
    const _fechaInicio = fechaInicio || moment(this.formularioFiltro.value.fechaInicio).format('DD/MM/YYYY');
    const _fechaFin = fechaFin || moment(this.formularioFiltro.value.fechaFin).format('DD/MM/YYYY');
